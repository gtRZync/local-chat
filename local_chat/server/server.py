...
from local_chat.command.data_loader import fetch_users_and_conversations
from local_chat.command.handle_messages import (
    create_conversations_for_connected_user,
...
class Server(socket.socket):
    ...
    def _authenticate_client(self, client_socket: socket.socket) -> int | None:
        ...
        try:
            ...
            event_data = f'EVENT:user.status.changed:{user_id}:online'
            for id, client in self.active_clients.items():
                if id != user_id:
                    client.sendall(event_data.encode())
            ...
            try:
                ...
                if message_data.startswith("MSG:"):
                    ...
                    try:
                        ...
                        # Store message in memory (for server-side tracking)
                        self._store_message(user_id, receiver_id, message_content)

                        save_message_to_conversation(user_id, receiver_id, message_content)
    ...
    def _disconnect_client(self, user_id: int, client_address: Address):
        ...
        if user_id in self.active_clients:
            try:
                event_data = f'EVENT:user.status.changed:{user_id}:offline'
                for id, client in self.active_clients.items():
                    if id != user_id:
                        client.sendall(event_data.encode())
        ...
        try:
            while True:
                try:
                    ...
                    # Create conversations with all other connected users
                    # This ensures they appear in each other's contact lists
                    existing_user_ids = [
                        uid for uid in self.active_clients.keys()
                        if uid != user_id
                    ]
                    if existing_user_ids:
                        # Create conversations between new user and all existing users
                        # Conversations are bidirectional, so one call per pair is enough
                        creation_status = create_conversations_for_connected_user(user_id, existing_user_ids)
                        print(creation_status)
                        if creation_status.created:
                            event_data: str = 'EVENT:contact.created'
                            for _, active_client in self.active_clients.items():
                                active_client.sendall(event_data.encode())
                        #TODO: maybe handle creation_status.op_failed
